	...
	: _battleChar(battleChar), UI(uiDeque)
{
	...
	auto expData = Application::Instance().GetDataBase().GetExpData(status.level);
	_addExp = expData.getPoint * (battleChar.GetGivenDamage() / static_cast<float>(targetStatus.health));
...
void Experience::Draw()
{
	...
	if (_updater == &Experience::LevelUpUpdate)
	{
		...
		auto fontH = Application::Instance().GetFileSystem()->GetFontHandle("choplin40");
		char str[10];
